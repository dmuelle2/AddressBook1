...
public class AddressListView {
	...
	public void setController(AddressList addressList) {
		...
		// EntryData page contains a list of Entries
		//  Created when user clicks "View Entries"
		public void viewAddressData(List<EntryData> entries) {
			...
			makeAddressTable(entries, flowPanel, dataListPanel);
		}
		...
		private FormPanel makeSubmitAddressForm(final EntryData entries){
			...
			VerticalPanel addressFormPanel = new VerticalPanel();
			addressFormPanel.addStyleName("submitPostVertPanel");
			submitFormPanel.add(addressFormPanel);
			...
			addressFormPanel.add(addressRow);
...
*/
			...
			addressFormPanel.add(addContactButton);
			...
			addContactButton.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					if(entries == null)
						control.handleAddressFromSubmitForm(submitFormPanel);
					else 
						control.handleAddressEditFromSubmitForm(entries, submitFormPanel);
				}});
			...
			submitFormPanel.addSubmitHandler(new FormPanel.SubmitHandler() {
				public void onSubmit(SubmitAddress event) {
					...
					if (zipTextbox.getText().length() == 0 || Double.isNaN(zip)) {
					...
					if (phoneTextbox.getText().length() == 0 || Double.isNaN(phone)) {
						Window.alert("A phone number is required. It must be a valid number");
						event.cancel();
					}
					if (upload.getFilename().length() == 0 && entries == null) {
			...
			submitFormPanel.addSubmitCompleteHandler(new FormPanel.SubmitCompleteHandler() {
				public void onSubmitComplete(SubmitCompleteEvent event) {
					...
					else control.viewAddressDataFromServer();
				}
		...
		private void makeAddressTable(List<EntryData> entries, FlowPanel flowPanel, VerticalPanel mainPanel) {
			for(EntryData entry: entries){
				if(entry !=null)
					flowPanel.add(makeAddressRow(entry, mainPanel));
				else System.out.println("null post");
			}
		}

		private HorizontalPanel makeAddressRow(final EntryData entries, final VerticalPanel mainPanel) {
			...
			deleteButton.addStyleName("addressInfoButton");
			...
			editButton.addStyleName("addressInfoButton");
			editButton.setText("Edit");
			//add a clickListener to the button
			editButton.addClickHandler(new ClickHandler() {
				@Override
				public void onClick(ClickEvent event) {
					viewAddressEditForm(entries);
				}
		      });
...
**/
		public void sendSuccessfulSavemessage() {
			Window.alert("Address was successfully stored.");
		}

		public void sendSuccessfulDeleteMessage() {
			Window.alert("Address was deleted successfully");
		}  
